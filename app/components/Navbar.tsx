...
export const Navbar = ({isUser}:Props) => {
  return (
<header className="sticky inset-0 z-50 border-b border-slate-100 bg-white/80 backdrop-blur-lg">
    <nav className="mx-auto flex lg:w-10/12 gap-2 lg:gap-8 px-6 transition-all duration-200 ease-in-out lg:px-12 py-4">
        <div className="relative flex items-center">
            <a href="/">
                <h1 className="scroll-m-20 text-xl font-extrabold uppercase tracking-wide lg:text-2xl ">Eclipse</h1>
             </a>
        </div>
        <ul className="hidden items-center justify-center gap-2 lg:gap-6 md:flex">
            {routeList.map((route) => {
                return(
                    <li key={route.label} className="pt-1.5 font-dm text-sm font-medium text-slate-700 text-shadow-[0_4px_8px_#bdbdbd]">
        ...
        <div className="hidden items-center justify-center  gap-2 lg:gap-6 md:flex">
            <a href="/" className="font-dm text-sm font-medium text-foreground underline">Contacto</a>
            <DynamicLinks isUser={isUser} />
        </div>
        <Sheet>
